...
engine::gui::FramedSprite::LayoutConfig::UVs buildUVsForButtonTexture()
{
...
engine::gui::FramedSprite::LayoutConfig buildLayoutConfigForButtonTexture()
{
...
{
    return engine::gui::TextureLibrary::instance().get(name);
}
...
EventStatus ButtonBase::on(const event::MouseButtonPressed& mouseButtonPressedEvent)
{
    ...
    bool isLeftClicked = mouseButtonPressedEvent.button == engine::gui::event::MouseButton::Left;
...
EventStatus ButtonBase::on(const event::MouseButtonReleased& mouseButtonReleasedEvent)
{
    ...
    bool isLeftReleased = mouseButtonReleasedEvent.button == engine::gui::event::MouseButton::Left;
    if (isLeftReleased)
    {
...
EventStatus ButtonBase::on(const event::MouseButtonDoublePressed& mouseButtonDoublePressedEvent)
{
    ...
    bool isLeftDoubleClicked = mouseButtonDoublePressedEvent.button == engine::gui::event::MouseButton::Left;
...
: ButtonBase{style}
{
    auto textPtr = engine::gui::Label::create(text, style_.text);
    text_ = textPtr.get();
    text_->setAlignment(engine::gui::Alignment::HorizontallyCentered | engine::gui::Alignment::VerticallyCentered);
...
: ButtonBase{style}
{
    auto icon  = engine::gui::Image::create(texture);
    icon_ = icon.get();

    auto layout = engine::gui::layout::Inset::create(engine::gui::layout::Constraint::Pixels(10.f));
    layout_ = layout.get();
