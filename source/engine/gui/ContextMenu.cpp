...
namespace
{
    ...
    engine::gui::FramedSprite::LayoutConfig::UVs buildUVsForMenu()
    {
    ...
    engine::gui::FramedSprite::LayoutConfig buildLayoutConfigForMenuTexture()
    {
        static auto layout = [] {
            engine::gui::FramedSprite::LayoutConfig lc;
    ...
    {
        const auto& style = engine::gui::style::StyleFactory::instance().button.text;

        auto measure = engine::gui::Label::create("", style);
...
void ContextMenu::buildMenu()
{
    ...
    auto vertical = engine::gui::layout::Vertical::create(items_.size());
    ...
    {
        vertical->setRowSize(index, engine::gui::layout::Constraint::Pixels(MENU_ENTRY_HEIGHT));

        auto menuEntry = engine::gui::TextButton::create(item.text, style::StyleFactory::instance().flatButton);
...
void ContextMenu::updateSubmenu(engine::gui::TextButton* buttonPtr, const Item& item, ptrdiff_t index)
{
