...
#include "engine/gui/FontHeightCache.hpp"
...
engine::gui::FramedSprite::LayoutConfig buildLayoutConfigForListBoxTexture()
{
    return engine::gui::FramedSprite::LayoutConfig{
        .cornerSizes = {.topLeft = {4.f, 4.f}, .bottomRight = {4.f, 4.f}},
        .uvs = engine::gui::FramedSprite::LayoutConfig::UVs{
...
, scrollValue_{1.f}
{
    ...
    auto scrollbar = engine::gui::scrollbar::Vertical::create();
...
EventStatus ListBox::on(const event::KeyboardKeyReleased& keyboardKeyReleasedEvent)
{
    ...
    if (keyboardKeyReleasedEvent.key == engine::gui::event::Key::Up)
    {
    ...
    if (keyboardKeyReleasedEvent.key == engine::gui::event::Key::Down)
    {
    ...
    if (keyboardKeyReleasedEvent.key == engine::gui::event::Key::Enter && selectedIndex_)
    {
