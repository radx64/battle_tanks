...
namespace
{
    ...
    float calculateButtonWidth(const std::string& text)
    {
        ...
        auto label = engine::gui::Label::create(text, style);
...
void MenuBar::setItems(const std::vector<engine::gui::ContextMenu::Item>& items)
{
    closeOpenMenu();
...
void MenuBar::rebuild()
{
    ...
    for (auto [index, entry] : std::views::enumerate(entries_))
    {
        ...
        auto button = engine::gui::TextButton::create(entry.item.text, style::StyleFactory::instance().flatButton);
...
void MenuBar::openMenuForEntry(ptrdiff_t index)
{
    ...
    auto menu = engine::gui::ContextMenu::create(entry.item.subItems);
    openMenu_ = menu.get();
    openMenuIndex_ = index;
    menu->setOnClose([this](engine::gui::ContextMenu* menuPtr) {
