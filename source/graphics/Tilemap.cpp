...
constexpr auto TILES_PER_CHUNK = CHUNK_SIZE / TILE_SIZE;
...
public:
    ...
    {
        sprite_.setTexture(graphics::TextureLibrary::instance().get(toTextureName(type)));
...
struct Tilemap::Impl
{
    ...
    using ChunkCoordinates = engine::ChunkedRenderTextureLayer::ChunkCoordinates;

    engine::ChunkedRenderTextureLayer::Chunk& getOrCreateChunk(const ChunkCoordinates& coordinates)
    {
        return chunks.getOrCreateChunk(
            coordinates,
            [this, &coordinates](auto& chunk)
            {
                const auto firstTileX = coordinates.first * TILES_PER_CHUNK;
                const auto firstTileY = coordinates.second * TILES_PER_CHUNK;
                const auto lastTileX = std::min(WIDTH, firstTileX + TILES_PER_CHUNK);
                const auto lastTileY = std::min(HEIGHT, firstTileY + TILES_PER_CHUNK);

                for (auto y = firstTileY; y < lastTileY; ++y)
                {
                    for (auto x = firstTileX; x < lastTileX; ++x)
                    {
                        auto tile = Tile(
                            static_cast<float>(x * TILE_SIZE) - chunk.position.x,
                            static_cast<float>(y * TILE_SIZE) - chunk.position.y,
                            idToType(tileIds[y][x]));
                        tile.draw(*chunk.target);
                    }
                }
            });
    }
    ...
    {
        return (WIDTH + TILES_PER_CHUNK - 1) / TILES_PER_CHUNK;
    }

    int getChunkCountY() const
    {
        return (HEIGHT + TILES_PER_CHUNK - 1) / TILES_PER_CHUNK;
    }

    engine::ChunkedRenderTextureLayer chunks;
    uint8_t tileIds[HEIGHT][WIDTH] =
    {
