...
float VerticalMousePolicy::translatePositionToThumbValue(const sf::Vector2f& mouse_position, const sf::Vector2f& trackPosition,
    const sf::Vector2f& trackSize, const sf::Vector2f& thumbSize, const float min, const float max, const float step)
{
    ...
    auto thumbYPositionOffset = trackPosition.y + trackSize.y - mouse_position.y - thumbSize.y / 2.f;
